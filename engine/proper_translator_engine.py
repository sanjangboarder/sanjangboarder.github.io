...
# Clean lexicon dictionary mapping
LEXICON = {
...
def translate_sentence_properly(line):
    raw = line.strip()
    if not raw:
        return ""
...
def translate_post_properly(ko_filepath):
    ...
    title_en = translate_sentence_properly(title_ko)
    if not title_en or len(title_en) < 5:
        title_en = f"Saltwater Fishing & Gear Review {fm.get('logNo', '')}"

    desc_en = translate_sentence_properly(desc_ko)[:160] if desc_ko else title_en
    ...
    for line in lines:
        if not line.strip():
            ...
            t = translate_sentence_properly(line)
            if t:
